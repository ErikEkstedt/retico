...
function SingleAnnotation(props) {
	...
	return (
		<div>
			<Plot
				data={[{ x: grades_x, y: grades_y, type: "bar" }]}
...
export default class Global extends Component {
	...
	componentDidMount() {
		fetch(baseUrl + "/tfo/" + this.state.interaction)
			.then((response) => response.json())
			.then((data) => {
				this.setState({ tfo: data });
			});

		fetch(baseUrl + "/fallback/" + this.state.interaction)
			.then((response) => response.json())
			.then((data) => {
				this.setState({ fallback: data.fallbacks });
			});

		fetch(baseUrl + "/omitted_turns/" + this.state.interaction)
			.then((response) => response.json())
			.then((data) => {
				this.setState({ omittedTurns: data.omitted_turns });
			});

		fetch(baseUrl + "/anno/" + this.state.interaction)
			.then((response) => response.json())
			.then((data) => {
				this.setState({ anno: data });
			});

		fetch(baseUrl + "/turn_opportunity/" + this.state.interaction)
			.then((response) => response.json())
			.then((data) => {
				this.setState({ turnOpportunity: data });
			});

		fetch(baseUrl + "/trp_info/" + this.state.interaction)
			.then((response) => response.json())
			.then((data) => {
		...
		fetch(
			baseUrl + "/responsiveness_and_interruption/" + this.state.interaction
		)
			.then((response) => response.json())
			.then((data) => {
	...
	getInfo() {
		...
		const n_fallbacks =
			this.state.fallback === null ? 0 : this.state.fallback.length;
		const n_omitted =
			this.state.omittedTurns === null ? 0 : this.state.omittedTurns.length;
		...
		if (this.state.responsivenessInterruption !== null) {
			...
			resp = (
				<div>
					...
					<p>
						Resp median: {this.state.responsivenessInterruption.time_median}{" "}
					</p>
		...
		return (
			<ListGroup>
				<ListGroup.Item>
					User turns: {this.state.trpInfo.user_turns}{" "}
				</ListGroup.Item>
				<ListGroup.Item>N trp: {this.state.trpInfo.trp.length}</ListGroup.Item>
				<ListGroup.Item>
					N fallbacks:{" "}
					{Math.round((n_fallbacks * 100) / this.state.trpInfo.user_turns)}% (
					{n_fallbacks})
				</ListGroup.Item>
				<ListGroup.Item>
					Agent turns: {this.state.trpInfo.agent_turns}
				</ListGroup.Item>
				<ListGroup.Item>
					Interruptions: {this.state.trpInfo.agent_abort_ratio * 100}% (
					{this.state.trpInfo.agent_aborted}). Interrupted agent turns where
					speech was emitted.
				</ListGroup.Item>
				<ListGroup.Item>
					Agent omitted: {n_omitted}. Turns without speech are omitted from the
					dialog (they never happened).{" "}
				</ListGroup.Item>
		...
		return (
			<div>
				...
				<Plot
					...
					layout={{
						xaxis: { nticks: 10, title: "TRP probability" },
						yaxis: { title: "%" },
					}}
				/>
			</div>
	...
	render() {
		...
		return (
			...
			>
				<Row>
					<Col xs={3} style={{ background: "#576c83" }}>
						{info}
					</Col>
					<Col>
						{tfo}
